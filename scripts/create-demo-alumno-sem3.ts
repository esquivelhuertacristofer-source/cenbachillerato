/**
 * Crea la cuenta demo de alumno para Semestre 3.
 * Uso: npx tsx scripts/create-demo-alumno-sem3.ts
 *
 * Requiere NEXT_PUBLIC_SUPABASE_URL y SUPABASE_SERVICE_ROLE_KEY en .env.local
 * Idempotente: re-ejecutar es seguro. No toca las cuentas alumno1-10 ni alumno-sem2.
 */
...
const DEMO_EMAIL    = "alumno-sem3@cenbachillerato-demo.com";
...
const DEMO_NOMBRE   = "Alumno Sem 3 Demo";
const DEMO_ESCUELA_CCT = "DEMO-001";
const DEMO_GRUPO_SEM3  = "Grupo 3A Demo";
...
async function getOrCreateGrupoSem3(sb: SupabaseClient<Database>, escuelaId: string): Promise<string> {
  const { data: existing } = await sb
    .from("grupos").select("id")
    .eq("nombre", DEMO_GRUPO_SEM3).eq("escuela_id", escuelaId).single();

  if (existing) {
    console.log(`  ↩  Grupo Sem 3 ya existe (id=${existing.id})`);
    return existing.id;
  }

  const { data, error } = await sb
    .from("grupos")
    .insert({ nombre: DEMO_GRUPO_SEM3, semestre: 3, escuela_id: escuelaId })
    .select("id").single();

  if (error || !data) throw new Error(`Error creando grupo Sem 3: ${error?.message}`);
  console.log(`  ✓  Grupo "${DEMO_GRUPO_SEM3}" creado (id=${data.id})`);
  return data.id;
}
...
async function upsertProfile(sb: SupabaseClient<Database>, userId: string, escuelaId: string): Promise<void> {
  ...
  console.log(`  ✓  Profile upsertado (semestre=3)`);
}
...
async function assignToGrupo(sb: SupabaseClient<Database>, userId: string, grupoId: string): Promise<void> {
  ...
  console.log(`  ✓  Alumno asignado a "${DEMO_GRUPO_SEM3}"`);
}
...
async function main() {
  ...
  console.log("\n🚀 CEN Bachillerato — Demo Alumno Semestre 3\n");

  console.log("1. Escuela demo:");
  const escuelaId = await getOrCreateEscuela(sb);

  console.log("\n2. Grupo Semestre 3:");
  const grupoId = await getOrCreateGrupoSem3(sb, escuelaId);
  ...
  console.log("\n" + "=".repeat(60));
  console.log("✅ LISTO — Cuenta demo Semestre 3 creada/verificada");
  console.log("=".repeat(60));
  console.log(`  Email:    ${DEMO_EMAIL}`);
  console.log(`  Password: ${DEMO_PASSWORD}`);
  console.log(`  Semestre: 3`);
  console.log(`  Grupo:    ${DEMO_GRUPO_SEM3}`);
  console.log("=".repeat(60) + "\n");
}
