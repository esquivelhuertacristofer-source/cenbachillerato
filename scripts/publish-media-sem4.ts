 ...
 * de Semestre 4 servidas desde public/media/sem4/. Recorre cada subcarpeta
 ...
 * cambia. Mismo patrón que publish-media-sem3.ts (Sem4 tampoco tiene items
 * video_con_preguntas, por eso no aparece en el mapa).
 *
 * Uso: npx tsx scripts/publish-media-sem4.ts
 */
...
const MEDIA_ROOT = resolve(process.cwd(), "public/media/sem4");
...
const CARPETA_CAMPO: Record<string, string> = {
  ...
  // Sem4 no tiene actividades video_con_preguntas, ni ejercicio_matematico suelto
  // (las 11 de este tipo son todas labs, cubiertas vía practica_slug → labs/).
};
...
async function main() {
  ...
  for (const [carpeta, campo] of Object.entries(CARPETA_CAMPO)) {
    ...
    for (const archivo of archivos) {
      ...
      const rutaPublica = `/media/sem4/${carpeta}/${archivo}`;
  ...
  log(`\n✅ Publicación Sem4: ${ok} nuevas, ${sinCambio} ya al día, ${noEncontrada} sin actividad, ${fallo} fallidas.\n`);
