 ...
 * interacciones distintas, todas sobre la crisis de la pandemia de COVID-19 que
 * analiza la lectura A1:
...
type Modo = "clasificar" | "actores" | "glosario";
...
export function LabCrisisSociales({ color }: PracticaLabProps) {
  ...
  const estrellas = (clasificarDone ? 1 : 0) + (actoresDone ? 1 : 0) + (glosarioDone ? 1 : 0);
  ...
  const resetActual = modo === "clasificar" ? resetClasificar : modo === "actores" ? resetActores : resetGlosario;
            ...
            <>
              <div style={{ ...card, padding: "18px 22px" }}>
                ...
                {glosLibres.length === 0 ? (
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                    {glosLibres.map((g) => (
                      <button key={g.id} className="cri-chip" data-sel={selGlos === g.id} onClick={() => setSelGlos((v) => (v === g.id ? null : g.id))} {...dragProps(g.id)}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              ...
              <div style={{ textAlign: "right", maxWidth: 180 }}>
                <div style={{ fontSize: 11.5, color: T.text3, lineHeight: 1.45 }}>
                  {bestEstrellas >= 3 ? "¡Analizas las crisis sociales como un científico social!" : "Completa los tres modos para ganar las estrellas."}
                </div>
              </div>
            </div>
          </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
      {PARES.map((g) => {
        ...
        return (
          ...
          >
            <div className="cri-slot" data-armed={!done && !!selGlos} style={done ? { borderStyle: "solid", borderColor: OK, background: `${OK}1a` } : undefined}>
              {done ? (
                <span style={{ animation: "criPop .25s ease", fontSize: 13, fontWeight: 900, color: "#fff", display: "inline-flex", alignItems: "center", gap: 7 }}>
