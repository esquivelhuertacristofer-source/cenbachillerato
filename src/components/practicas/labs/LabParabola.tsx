 ...
 * espejo. El alumno mueve a, b y c y ve cómo cambia la trayectoria del balón.
...
export function LabParabola({ color }: PracticaLabProps) {
  ...
  // objetivos
  const [vioVertice] = useState(true);
  ...
  const cambiarB = (v: number) => { setB(v); setMovioBC(true); };
  const cambiarC = (v: number) => { setC(v); setMovioBC(true); };
  const toggleGol = () => { setShowGol((g) => !g); setVioGol(true); };
  ...
  const objetivos = [
    { txt: "Identifica el vértice (punto más alto)", done: vioVertice },
            ...
            {showGol && (
              <div style={{ marginTop: 14, padding: "12px 14px", borderRadius: 12, background: `rgba(${color.rgba},0.08)`, border: `1px solid rgba(${color.rgba},0.28)` }}>
                ...
                <div style={{ fontSize: 12.5, color: T.text2, lineHeight: 1.5 }}>
                  {enAire ? (
                    <>Comparación: <strong style={{ color: balonSupera ? ROJO : VERDE, fontFamily: "ui-monospace, monospace" }}>{fmtNum(hPorteria, 2)} m {balonSupera ? ">" : "≤"} {fmtNum(GOL.travesano, 2)} m</strong>. {balonSupera
