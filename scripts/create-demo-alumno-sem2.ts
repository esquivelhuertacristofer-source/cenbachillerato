/**
 * Crea la cuenta demo de alumno para Semestre 2.
 * Uso: npx tsx scripts/create-demo-alumno-sem2.ts
 *
 * Requiere NEXT_PUBLIC_SUPABASE_URL y SUPABASE_SERVICE_ROLE_KEY en .env.local
 * Idempotente: re-ejecutar es seguro. No toca las cuentas alumno1-10.
 */
...
async function getOrCreateGrupoSem2(sb: SupabaseClient<Database>, escuelaId: string): Promise<string> {
  const { data: existing } = await sb
    .from("grupos").select("id")
    .eq("nombre", DEMO_GRUPO_SEM2).eq("escuela_id", escuelaId).single();

  if (existing) {
    console.log(`  ↩  Grupo Sem 2 ya existe (id=${existing.id})`);
    return existing.id;
  }

  const { data, error } = await sb
    .from("grupos")
    .insert({ nombre: DEMO_GRUPO_SEM2, semestre: 2, escuela_id: escuelaId })
    .select("id").single();

  if (error || !data) throw new Error(`Error creando grupo Sem 2: ${error?.message}`);
  console.log(`  ✓  Grupo "${DEMO_GRUPO_SEM2}" creado (id=${data.id})`);
  return data.id;
}
...
async function upsertProfile(sb: SupabaseClient<Database>, userId: string, escuelaId: string): Promise<void> {
  ...
  console.log(`  ✓  Profile upsertado (semestre=2)`);
}
...
async function assignToGrupo(sb: SupabaseClient<Database>, userId: string, grupoId: string): Promise<void> {
  ...
  console.log(`  ✓  Alumno asignado a "${DEMO_GRUPO_SEM2}"`);
}
...
async function main() {
  ...
  console.log("\n🚀 CEN Bachillerato — Demo Alumno Semestre 2\n");

  console.log("1. Escuela demo:");
  const escuelaId = await getOrCreateEscuela(sb);

  console.log("\n2. Grupo Semestre 2:");
  const grupoId = await getOrCreateGrupoSem2(sb, escuelaId);
  ...
  console.log("\n" + "=".repeat(60));
  console.log("✅ LISTO — Cuenta demo Semestre 2 creada/verificada");
  console.log("=".repeat(60));
  console.log(`  Email:    ${DEMO_EMAIL}`);
  console.log(`  Password: ${DEMO_PASSWORD}`);
  console.log(`  Semestre: 2`);
  console.log(`  Grupo:    ${DEMO_GRUPO_SEM2}`);
  console.log("=".repeat(60) + "\n");
}
