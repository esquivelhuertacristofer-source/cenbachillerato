...
import { DISCRIMINANTE_FICHA } from "./discriminante-ficha";
...
import {
  ...
  fmtDelta, fmtNum, fmtCoef, RETO_A2, type Caso, type Escenario,
...
export function LabDiscriminante({ color }: PracticaLabProps) {
  ...
  const aplicar = (e: Escenario) => { setA(e.a); setB(e.b); setC(e.c); setK(e.k); bump(); if (sonido) audioRef.current?.blip(); };
    ...
    <div style={{ color: T.text }}>
      ...
      <aside className="ex-drawer" data-open={drawer} aria-hidden={!drawer}>
        ...
        <div className="ex-drawer-body">
          <FichaTeorica data={DISCRIMINANTE_FICHA} accent={accent} rgba={color.rgba} defaultOpen />
        </div>
      </aside>
    </div>
  );
