 ...
 *
 * Interactividad máxima. Cuatro modos: los tres de arrastrar/clasificar (rol · ambiente · término) y,
 ...
 *     de memoria el término del glosario que la nombra. El VOCABULARIO.
...
type Modo = "personajes" | "escenarios" | "glosario" | "texto";
...
export function LabPersonajesEscenarios({ color }: PracticaLabProps) {
  ...
  const modosHechos = (personajesDone ? 1 : 0) + (escenariosDone ? 1 : 0) + (glosarioDone ? 1 : 0) + (textoDone ? 1 : 0);
  ...
  const resetActual = modo === "texto" ? resetTexto : modo === "personajes" ? resetPersonajes : modo === "escenarios" ? resetEscenarios : resetGlosario;
  ...
  return (
    <div style={{ color: T.text }}>
      <style>{`
        ...
        /* Cajón de teoría */
        .pe-scrim { position:fixed; inset:0; background:rgba(2,8,20,0.55); backdrop-filter:blur(2px);
          opacity:0; pointer-events:none; transition:opacity .3s ease; z-index:60; }
        .pe-scrim[data-open="true"] { opacity:1; pointer-events:auto; }
        .pe-drawer { position:fixed; top:0; right:0; height:100dvh; width:min(560px,94vw); z-index:61;
          background:linear-gradient(180deg,#06182f 0%,#020d1d 100%); border-left:1px solid rgba(${color.rgba},0.32);
          box-shadow:-24px 0 60px -20px rgba(0,0,0,0.7); transform:translateX(102%); transition:transform .34s cubic-bezier(.4,0,.2,1);
          display:flex; flex-direction:column; }
        .pe-drawer[data-open="true"] { transform:translateX(0); }
        .pe-drawer-head { display:flex; align-items:center; justify-content:space-between; gap:12px;
          padding:18px 20px; border-bottom:1px solid ${T.line}; }
        .pe-drawer-body { overflow-y:auto; padding:20px; flex:1; }
        .pe-close { cursor:pointer; width:36px; height:36px; border-radius:10px; border:1px solid ${T.line};
          background:${T.glass}; color:#fff; font-size:15px; display:flex; align-items:center; justify-content:center; transition:all .15s; }
        .pe-close:hover { border-color:${accent}; background:rgba(${color.rgba},0.16); }
        .pe-teoria-fab { position:fixed; right:20px; bottom:20px; z-index:58; cursor:pointer; display:inline-flex; align-items:center; gap:9px;
          padding:11px 16px; border-radius:999px; border:1px solid ${accent}88; color:#fff; font-size:13px; font-weight:800;
          background:rgba(2,12,28,0.86); backdrop-filter:blur(10px); box-shadow:0 8px 28px -8px ${accent}; transition:all .16s; }
        .pe-teoria-fab:hover { background:rgba(${color.rgba},0.28); transform:translateY(-1px); }
        @media (max-width: 640px){ .pe-teoria-fab { right:12px; bottom:12px; padding:10px 13px; font-size:12px; } }

        /* Identidad del tablero */
        .pe-bin, .pe-row { --tono:188; position:relative;
          background-image:radial-gradient(120% 90% at 0% 0%, hsl(var(--tono) 72% 58% / 0.11) 0%, transparent 62%); }
        .pe-bin:nth-of-type(6n+1), .pe-row:nth-of-type(6n+1) { --tono:188; }
        .pe-bin:nth-of-type(6n+2), .pe-row:nth-of-type(6n+2) { --tono:262; }
        .pe-bin:nth-of-type(6n+3), .pe-row:nth-of-type(6n+3) { --tono:44; }
        .pe-bin:nth-of-type(6n+4), .pe-row:nth-of-type(6n+4) { --tono:152; }
        .pe-bin:nth-of-type(6n+5), .pe-row:nth-of-type(6n+5) { --tono:330; }
        .pe-bin:nth-of-type(6n+6), .pe-row:nth-of-type(6n+6) { --tono:18; }
        .pe-bin::before, .pe-row::before { content:""; position:absolute; top:0; left:10px; right:10px; height:3px; border-radius:0 0 3px 3px;
          background:linear-gradient(90deg, hsl(var(--tono) 78% 62%) 0%, hsl(var(--tono) 78% 62% / 0.15) 100%); }
        .pe-bin[data-done="true"], .pe-row[data-done="true"] {
          background-image:radial-gradient(120% 90% at 0% 0%, hsl(var(--tono) 72% 58% / 0.2) 0%, transparent 68%); }
        .pe-chip { transition:transform .14s, box-shadow .14s, border-color .14s, background .14s; }
        .pe-chip:hover { transform:translateY(-2px); }
        .pe-chip[data-sel="true"] { transform:translateY(-3px) scale(1.02); }
        @media (prefers-reduced-motion: reduce){
          .pe-chip, .pe-chip:hover, .pe-chip[data-sel="true"] { transform:none; transition:none; }
        }
      `}</style>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 18 }}>
        ...
        <button className="pe-icobtn" data-on={drawer} onClick={() => setDrawer(true)} title="Teoría de la práctica">
      ...
      {/* ── Cajón de teoría ──────────────────────────────────────────── */}
      <button className="pe-teoria-fab" onClick={() => setDrawer(true)}>
        <i className="fa-solid fa-book-open" />
        Teoría
      </button>
      <div className="pe-scrim" data-open={drawer} onClick={() => setDrawer(false)} />
      <aside className="pe-drawer" data-open={drawer} aria-hidden={!drawer}>
        <div className="pe-drawer-head">
          <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
            <i className="fa-solid fa-book-open" style={{ color: accent, fontSize: 17 }} />
            <span style={{ fontSize: 15, fontWeight: 900, color: T.text }}>Teoría de la práctica</span>
          </div>
          <button className="pe-close" onClick={() => setDrawer(false)} title="Cerrar">
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              ...
              <div style={{ textAlign: "right", maxWidth: 180 }}>
                <div style={{ fontSize: 11.5, color: T.text3, lineHeight: 1.45 }}>
                  {bestEstrellas >= 3 ? "¡Dominas a los personajes y sus escenarios!" : "Termina los tres modos para ganar 2★; la tercera pide 2 errores o menos."}
                </div>
              </div>
            </div>
          </div>
