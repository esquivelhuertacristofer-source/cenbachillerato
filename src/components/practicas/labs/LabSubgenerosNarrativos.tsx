  ...
  return (
    <div style={{ color: T.text }}>
      <style>{`
        ...
        .sn-teoria-fab { position:fixed; right:20px; bottom:20px; z-index:58; cursor:pointer; display:inline-flex; align-items:center; gap:9px;
          padding:11px 16px; border-radius:999px; border:1px solid ${accent}88; color:#fff; font-size:13px; font-weight:800;
          background:rgba(2,12,28,0.86); backdrop-filter:blur(10px); box-shadow:0 8px 28px -8px ${accent}; transition:all .16s; }
        .sn-teoria-fab:hover { background:rgba(${color.rgba},0.28); transform:translateY(-1px); }
        @media (max-width: 640px){ .sn-teoria-fab { right:12px; bottom:12px; padding:10px 13px; font-size:12px; } }

        /* Identidad del tablero */
        .sn-bin, .sn-row { --tono:188; position:relative;
          background-image:radial-gradient(120% 90% at 0% 0%, hsl(var(--tono) 72% 58% / 0.11) 0%, transparent 62%); }
        .sn-bin:nth-of-type(6n+1), .sn-row:nth-of-type(6n+1) { --tono:188; }
        .sn-bin:nth-of-type(6n+2), .sn-row:nth-of-type(6n+2) { --tono:262; }
        .sn-bin:nth-of-type(6n+3), .sn-row:nth-of-type(6n+3) { --tono:44; }
        .sn-bin:nth-of-type(6n+4), .sn-row:nth-of-type(6n+4) { --tono:152; }
        .sn-bin:nth-of-type(6n+5), .sn-row:nth-of-type(6n+5) { --tono:330; }
        .sn-bin:nth-of-type(6n+6), .sn-row:nth-of-type(6n+6) { --tono:18; }
        .sn-bin::before, .sn-row::before { content:""; position:absolute; top:0; left:10px; right:10px; height:3px; border-radius:0 0 3px 3px;
          background:linear-gradient(90deg, hsl(var(--tono) 78% 62%) 0%, hsl(var(--tono) 78% 62% / 0.15) 100%); }
        .sn-bin[data-done="true"], .sn-row[data-done="true"] {
          background-image:radial-gradient(120% 90% at 0% 0%, hsl(var(--tono) 72% 58% / 0.2) 0%, transparent 68%); }
        .sn-chip { transition:transform .14s, box-shadow .14s, border-color .14s, background .14s; }
        .sn-chip:hover { transform:translateY(-2px); }
        .sn-chip[data-sel="true"] { transform:translateY(-3px) scale(1.02); }
        @media (prefers-reduced-motion: reduce){
          .sn-chip, .sn-chip:hover, .sn-chip[data-sel="true"] { transform:none; transition:none; }
        }
      `}</style>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 18 }}>
        ...
        <button className="sn-icobtn" data-on={drawer} onClick={() => setDrawer(true)} title="Teoría de la práctica">
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              ...
              <div style={{ textAlign: "right", maxWidth: 180 }}>
                <div style={{ fontSize: 11.5, color: T.text3, lineHeight: 1.45 }}>
                  {bestEstrellas >= 3 ? "¡Distingues los subgéneros narrativos como un crítico!" : "Termina los tres modos para ganar 2★; la tercera pide 2 errores o menos."}
                </div>
              </div>
            </div>
          </div>
