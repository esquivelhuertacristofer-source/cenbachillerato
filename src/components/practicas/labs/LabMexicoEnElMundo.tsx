...
type Modo = "linea" | "siglos" | "glosario" | "texto";
...
export function LabMexicoEnElMundo({ color }: PracticaLabProps) {
  ...
  const resetActual = modo === "texto" ? resetTexto : modo === "linea" ? resetLinea : modo === "siglos" ? resetSiglos : resetGlosario;
  ...
  return (
    <div style={{ color: T.text }}>
      <style>{`
        ...
        /* Cajón de teoría */
        .mem-scrim { position:fixed; inset:0; background:rgba(2,8,20,0.55); backdrop-filter:blur(2px);
          opacity:0; pointer-events:none; transition:opacity .3s ease; z-index:60; }
        .mem-scrim[data-open="true"] { opacity:1; pointer-events:auto; }
        .mem-drawer { position:fixed; top:0; right:0; height:100dvh; width:min(560px,94vw); z-index:61;
          background:linear-gradient(180deg,#06182f 0%,#020d1d 100%); border-left:1px solid rgba(${color.rgba},0.32);
          box-shadow:-24px 0 60px -20px rgba(0,0,0,0.7); transform:translateX(102%); transition:transform .34s cubic-bezier(.4,0,.2,1);
          display:flex; flex-direction:column; }
        .mem-drawer[data-open="true"] { transform:translateX(0); }
        .mem-drawer-head { display:flex; align-items:center; justify-content:space-between; gap:12px;
          padding:18px 20px; border-bottom:1px solid ${T.line}; }
        .mem-drawer-body { overflow-y:auto; padding:20px; flex:1; }
        .mem-close { cursor:pointer; width:36px; height:36px; border-radius:10px; border:1px solid ${T.line};
          background:${T.glass}; color:#fff; font-size:15px; display:flex; align-items:center; justify-content:center; transition:all .15s; }
        .mem-close:hover { border-color:${accent}; background:rgba(${color.rgba},0.16); }
        .mem-teoria-fab { position:fixed; right:20px; bottom:20px; z-index:58; cursor:pointer; display:inline-flex; align-items:center; gap:9px;
          padding:11px 16px; border-radius:999px; border:1px solid ${accent}88; color:#fff; font-size:13px; font-weight:800;
          background:rgba(2,12,28,0.86); backdrop-filter:blur(10px); box-shadow:0 8px 28px -8px ${accent}; transition:all .16s; }
        .mem-teoria-fab:hover { background:rgba(${color.rgba},0.28); transform:translateY(-1px); }
        @media (max-width: 640px){ .mem-teoria-fab { right:12px; bottom:12px; padding:10px 13px; font-size:12px; } }
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 18 }}>
        ...
        <button className="mem-icobtn" data-on={drawer} onClick={() => setDrawer(true)} title="Teoría de la práctica">
      ...
      {/* ── Cajón de teoría ──────────────────────────────────────────── */}
      <button className="mem-teoria-fab" onClick={() => setDrawer(true)}>
        <i className="fa-solid fa-book-open" />
        Teoría
      </button>
      <div className="mem-scrim" data-open={drawer} onClick={() => setDrawer(false)} />
      <aside className="mem-drawer" data-open={drawer} aria-hidden={!drawer}>
        <div className="mem-drawer-head">
          <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
            <i className="fa-solid fa-book-open" style={{ color: accent, fontSize: 17 }} />
            <span style={{ fontSize: 15, fontWeight: 900, color: T.text }}>Teoría de la práctica</span>
          </div>
          <button className="mem-close" onClick={() => setDrawer(false)} title="Cerrar">
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              ...
              <div style={{ textAlign: "right", maxWidth: 180 }}>
                <div style={{ fontSize: 11.5, color: T.text3, lineHeight: 1.45 }}>
                  {bestEstrellas >= 3 ? "¡Conectas la historia de México con el mundo!" : "Termina los tres modos para ganar 2★; la tercera pide 2 errores o menos."}
                </div>
              </div>
            </div>
          </div>
