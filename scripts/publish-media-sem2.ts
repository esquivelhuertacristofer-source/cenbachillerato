 ...
 * de Semestre 2 servidas desde public/media/sem2/. Recorre cada subcarpeta
 ...
 * cambia. Mismo patrón que publish-media-sem1.ts (Sem2 no tiene items
 * video_con_preguntas, por eso no aparece en el mapa).
 *
 * Uso: npx tsx scripts/publish-media-sem2.ts
 */
...
const MEDIA_ROOT = resolve(process.cwd(), "public/media/sem2");
...
const CARPETA_CAMPO: Record<string, string> = {
  ...
  // Sem2 no tiene actividades video_con_preguntas.
};
...
async function main() {
  ...
  for (const [carpeta, campo] of Object.entries(CARPETA_CAMPO)) {
    ...
    for (const archivo of archivos) {
      ...
      const rutaPublica = `/media/sem2/${carpeta}/${archivo}`;
  ...
  log(`\n✅ Publicación Sem2: ${ok} nuevas, ${sinCambio} ya al día, ${noEncontrada} sin actividad, ${fallo} fallidas.\n`);
