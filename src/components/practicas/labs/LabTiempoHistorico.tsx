 ...
 *  3. «Empareja tipo de tiempo y definición» — arrastra cada concepto del
 *     glosario a su definición verbatim (A5).
...
type Modo = "duracion" | "linea" | "glosario";
...
const MODOS: { id: Modo; label: string; icono: string }[] = [
  ...
  { id: "glosario", label: "Empareja tipo de tiempo y definición", icono: "fa-book-open" },
];
...
export function LabTiempoHistorico({ color }: PracticaLabProps) {
  ...
  const estrellas = (duracionDone ? 1 : 0) + (lineaDone ? 1 : 0) + (glosarioDone ? 1 : 0);
  ...
  const objetivos = [
    ...
    { txt: "Empareja los 6 conceptos del glosario", done: glosarioDone },
  ...
  const resetActual = modo === "duracion" ? resetDuracion : modo === "linea" ? resetLinea : resetGlosario;
          ...
          {modo === "glosario" && (
            <>
              <div style={{ ...card, padding: "18px 22px" }}>
                ...
                {glosLibres.length === 0 ? (
                  <div style={{ fontSize: 13.5, color: OK, fontWeight: 700, display: "flex", alignItems: "center", gap: 9 }}>
                    <i className="fa-solid fa-circle-check" /> ¡Emparejaste los {PARES.length} conceptos!
                  </div>
                ) : (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                    {glosLibres.map((g) => (
                      <button key={g.id} className="tih-chip" data-sel={selGlos === g.id} onClick={() => setSelGlos((s) => (s === g.id ? null : g.id))} {...dragProps(g.id)}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              ...
              <div style={{ textAlign: "right", maxWidth: 180 }}>
                <div style={{ fontSize: 11.5, color: T.text3, lineHeight: 1.45 }}>
                  {bestEstrellas >= 3 ? "¡Distingues el tiempo que medimos del que sentimos!" : "Completa los tres modos para ganar las estrellas."}
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ borderRadius: 18, padding: "16px 18px", border: `1px solid rgba(${color.rgba},0.3)`, background: `rgba(${color.rgba},0.08)`, fontSize: 13, color: T.text2, lineHeight: 1.55, display: "flex", gap: 12 }}>
            ...
            <span>
              ...
              {modo === "glosario" && (
                <>Lee primero la definición y su ejemplo; luego suelta el concepto que le corresponde.</>
              )}
            </span>
          </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
      {PARES.map((g) => {
        ...
        return (
          ...
          >
            <div className="tih-slot" data-armed={!done && !!selGlos} style={done ? { borderStyle: "solid", borderColor: OK, background: `${OK}1a` } : undefined}>
              {done ? (
                <span style={{ animation: "tihPop .25s ease", fontSize: 13, fontWeight: 900, color: "#fff", display: "inline-flex", alignItems: "center", gap: 7 }}>
