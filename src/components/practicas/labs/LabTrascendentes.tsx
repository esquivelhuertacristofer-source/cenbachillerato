...
export function LabTrascendentes({ color }: PracticaLabProps) {
  ...
  return (
    <div style={{ color: T.text }}>
      <style>{`
        @keyframes exPulseTra { 0%,100%{ box-shadow:0 0 0 0 var(--exc); } 50%{ box-shadow:0 0 0 6px transparent; } }
        .ex-live-dot { animation: exPulseTra 1.6s ease-in-out infinite; }
      ...
      <div style={{ marginTop: 16, fontSize: 11.5, color: T.text3, lineHeight: 1.5, display: "flex", gap: 9, alignItems: "flex-start" }}>
        ...
        <span>
          Cálculo <strong>exacto</strong>: las cuatro derivadas son <strong>simbólicas</strong> (las derivadas básicas y las reglas aplicadas a mano) y la tangente usa la pendiente real f&apos;(a). Los cuatro casos —<strong>3 sen x − 2 cos x + tan x</strong> (linealidad), <strong>e^(2x)·cos x</strong> (producto + cadena), <strong>ln(x² + 1)</strong> (cadena) y <strong>sen(x³)</strong> (cadena)— son verbatim del enunciado A2. El plano dibuja f y f&apos; a una escala propia por caso para que ambas curvas quepan; en la tangente (tan x) el plano se queda dentro de ±1.05 rad para no tocar la asíntota de x = π/2.
        </span>
      </div>

      {/* ── Reto evaluable: ejercicio verbatim PM-V-P05-A2 ────────────── */}
      <RetoNumericoCard
        reto={RETO_A2}
        accent={accent}
