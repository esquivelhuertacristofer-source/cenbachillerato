...
/**
 * Escena 3D — "La función y su derivada: las cuatro reglas" (PM-V-P04-A2).
 *
 ...
 * La ALTURA de D coincide con la PENDIENTE de la tangente en P: derivar (con las
 * cuatro reglas) es justamente construir esa curva f'.
 *
...
export interface ReglasSceneProps {
  funcId: FuncId;
...
function SondaActiva({ funcId, v, aPos }: { funcId: FuncId; v: Vista; aPos: number }) {
  ...
  return (
    <group>
      ...
      {tanSeg.length === 2 && (
        <CurvaTubo puntos={[tanSeg[0]!, tanSeg[1]!]} color={TAN_COL} grosor={0.058} />
      )}
...
function Contenido({ funcId, aPos, accent, resetNonce }: ReglasSceneProps) {
  const f = func(funcId);
  const v = f.vista;
...
export default function ReglasScene(props: ReglasSceneProps) {
  return (
