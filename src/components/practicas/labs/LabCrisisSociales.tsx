 ...
 *
 * Interactividad máxima. Cuatro modos sobre la crisis de la pandemia de
 * COVID-19 que analiza la lectura A1: tres de arrastrar/clasificar y, al final,
 * uno que se escribe («Completa el texto», verbatim de la progresión):
 *  1. «¿Causa, actor o consecuencia?» — clasifica nueve elementos de la crisis
 *     según el principio de A1: las crisis no son accidentales, sino el cruce de
 *     causas estructurales, actores sociales y consecuencias diferenciadas.
 *  2. «Empareja actor y su papel» — arrastra cada actor social a la función que
 *     cumplió durante la pandemia (verbatim A1).
 ...
 * clic-para-seleccionar / clic-para-colocar). Contenido VERBATIM de CS-III·P01.
 */
...
type Modo = "clasificar" | "actores" | "glosario" | "texto";
...
export function LabCrisisSociales({ color }: PracticaLabProps) {
  ...
  const resetActual = modo === "texto" ? resetTexto : modo === "clasificar" ? resetClasificar : modo === "actores" ? resetActores : resetGlosario;
  ...
  return (
    <div style={{ color: T.text }}>
      <style>{`
        ...
        .cri-tab { cursor:pointer; display:inline-flex; align-items:center; gap:9px; padding:10px 16px; border-radius:11px;
          border:1px solid ${T.line}; background:${T.glass}; color:${T.text2}; font-size:13.5px; font-weight:800; transition:all .14s; }
        .cri-tab:hover { border-color:${T.lineStrong}; color:#fff; }
        .cri-tab[data-on="true"] { border-color:${accent}; background:rgba(${color.rgba},0.16); color:#fff; box-shadow:0 0 16px -6px ${accent}; }
        .cri-icobtn { cursor:pointer; width:36px; height:36px; border-radius:9px; display:flex; align-items:center; justify-content:center;
          font-size:14px; border:1px solid ${T.line}; background:${T.glass}; color:rgba(255,255,255,0.7); transition:all .15s; }
        .cri-icobtn[data-on="true"] { background:rgba(${color.rgba},0.22); color:#fff; border-color:${accent}; }
        .cri-icobtn:hover { background:rgba(255,255,255,0.12); }
        ...
        .cri-chip[data-sel="true"] { border-color:${accent}; background:rgba(${color.rgba},0.2); box-shadow:0 0 16px -5px ${accent}; }
        ...
        .cri-slot { flex-shrink:0; min-width:210px; min-height:42px; border-radius:11px; border:1.5px dashed ${T.lineStrong}; background:${T.inset};
        ...
        .cri-slot[data-armed="true"] { border-color:${accent}; background:rgba(${color.rgba},0.1); }
        ...
        .cri-btn:hover { border-color:${T.lineStrong}; }
        .cri-divider { height:1px; background:${T.line}; margin:18px 0; }
        @media (prefers-reduced-motion: reduce){ .cri-row[data-shake="true"], .cri-bin[data-shake="true"] { animation:none; } }

        /* Cajón de teoría */
        .cri-scrim { position:fixed; inset:0; background:rgba(2,8,20,0.55); backdrop-filter:blur(2px);
          opacity:0; pointer-events:none; transition:opacity .3s ease; z-index:60; }
        .cri-scrim[data-open="true"] { opacity:1; pointer-events:auto; }
        .cri-drawer { position:fixed; top:0; right:0; height:100dvh; width:min(560px,94vw); z-index:61;
          background:linear-gradient(180deg,#06182f 0%,#020d1d 100%); border-left:1px solid rgba(${color.rgba},0.32);
          box-shadow:-24px 0 60px -20px rgba(0,0,0,0.7); transform:translateX(102%); transition:transform .34s cubic-bezier(.4,0,.2,1);
          display:flex; flex-direction:column; }
        .cri-drawer[data-open="true"] { transform:translateX(0); }
        .cri-drawer-head { display:flex; align-items:center; justify-content:space-between; gap:12px;
          padding:18px 20px; border-bottom:1px solid ${T.line}; }
        .cri-drawer-body { overflow-y:auto; padding:20px; flex:1; }
        .cri-close { cursor:pointer; width:36px; height:36px; border-radius:10px; border:1px solid ${T.line};
          background:${T.glass}; color:#fff; font-size:15px; display:flex; align-items:center; justify-content:center; transition:all .15s; }
        .cri-close:hover { border-color:${accent}; background:rgba(${color.rgba},0.16); }
        .cri-teoria-fab { position:fixed; right:20px; bottom:20px; z-index:58; cursor:pointer; display:inline-flex; align-items:center; gap:9px;
          padding:11px 16px; border-radius:999px; border:1px solid ${accent}88; color:#fff; font-size:13px; font-weight:800;
          background:rgba(2,12,28,0.86); backdrop-filter:blur(10px); box-shadow:0 8px 28px -8px ${accent}; transition:all .16s; }
        .cri-teoria-fab:hover { background:rgba(${color.rgba},0.28); transform:translateY(-1px); }
        @media (max-width: 640px){ .cri-teoria-fab { right:12px; bottom:12px; padding:10px 13px; font-size:12px; } }
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 18 }}>
        {MODOS.map((m) => (
          <button key={m.id} className="cri-tab" data-on={modo === m.id} onClick={() => setModo(m.id)}>
        ...
        <button className="cri-icobtn" data-on={drawer} onClick={() => setDrawer(true)} title="Teoría de la práctica">
          <i className="fa-solid fa-book-open" />
        </button>
        <button className="cri-icobtn" data-on={sonido} onClick={toggleSonido} title={sonido ? "Silenciar" : "Activar sonido"}>
          <i className={`fa-solid ${sonido ? "fa-volume-high" : "fa-volume-xmark"}`} />
        </button>
        <button className="cri-icobtn" onClick={resetActual} title="Reiniciar este modo">
          <i className="fa-solid fa-rotate-left" />
        </button>
      </div>

      {/* ── Cajón de teoría ──────────────────────────────────────────── */}
      <button className="cri-teoria-fab" onClick={() => setDrawer(true)}>
        <i className="fa-solid fa-book-open" />
        Teoría
      </button>
      <div className="cri-scrim" data-open={drawer} onClick={() => setDrawer(false)} />
      <aside className="cri-drawer" data-open={drawer} aria-hidden={!drawer}>
        <div className="cri-drawer-head">
          <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
            <i className="fa-solid fa-book-open" style={{ color: accent, fontSize: 17 }} />
            <span style={{ fontSize: 15, fontWeight: 900, color: T.text }}>Teoría de la práctica</span>
          </div>
          <button className="cri-close" onClick={() => setDrawer(false)} title="Cerrar">
            <i className="fa-solid fa-xmark" />
          </button>
        </div>
        <div className="cri-drawer-body">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) clamp(300px,28vw,400px)", gap: 22, alignItems: "start" }}>
            ...
            <>
              <div style={{ ...card, padding: "18px 22px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12.5, fontWeight: 800, color: actoresDone ? OK : T.text3 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                    {actLibres.map((c) => (
                      <button key={c.id} className="cri-chip" data-sel={selAct === c.id} onClick={() => setSelAct((v) => (v === c.id ? null : c.id))} {...dragProps(c.id)}>
                        <i className="fa-solid fa-users" style={{ fontSize: 11, color: T.text3 }} />
                        {c.actor}
                      </button>
                    ))}
                  </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              ...
              <div style={{ textAlign: "right", maxWidth: 180 }}>
                <div style={{ fontSize: 11.5, color: T.text3, lineHeight: 1.45 }}>
                  {bestEstrellas >= 3 ? "¡Analizas las crisis sociales como un científico social!" : "Termina los tres modos para ganar 2★; la tercera pide 2 errores o menos."}
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ borderRadius: 18, padding: "16px 18px", border: `1px solid rgba(${color.rgba},0.3)`, background: `rgba(${color.rgba},0.08)`, fontSize: 13, color: T.text2, lineHeight: 1.55, display: "flex", gap: 12 }}>
            ...
            <span>
              {modo === "clasificar" && (
                <>Una <strong style={{ color: T.text }}>causa estructural</strong> es una condición previa de fondo; un <strong style={{ color: T.text }}>actor social</strong> es quien actúa frente a la crisis; una <strong style={{ color: T.text }}>consecuencia</strong> es cómo se vivió la crisis de forma desigual.</>
              )}
              {modo === "actores" && (
                <>Cada actor tuvo intereses y capacidades distintas: el <strong style={{ color: T.text }}>Estado</strong> contuvo, la <strong style={{ color: T.text }}>sociedad civil</strong> llenó vacíos, el <strong style={{ color: T.text }}>sector empresarial</strong> negoció y los <strong style={{ color: T.text }}>organismos internacionales</strong> orientaron.</>
              )}
              {modo === "glosario" && (
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {dentro.length === 0 ? (
                ...
                dentro.map((e) => (
                  <span key={e.id} style={{ animation: "criPop .25s ease", display: "inline-flex", alignItems: "flex-start", gap: 7, padding: "8px 12px", borderRadius: 11, background: `${OK}1a`, border: `1px solid ${OK}55`, fontSize: 12.5, fontWeight: 700, color: "#fff", lineHeight: 1.4 }}>
        ...
        return (
          ...
          >
            <div className="cri-slot" data-armed={!done && !!selAct} style={done ? { borderStyle: "solid", borderColor: OK, background: `${OK}1a` } : undefined}>
              {done ? (
                <span style={{ animation: "criPop .25s ease", fontSize: 13, fontWeight: 900, color: "#fff", display: "inline-flex", alignItems: "center", gap: 7 }}>
                  <i className="fa-solid fa-users" />
                  {c.actor}
                </span>
              ) : (
                <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                  <i className="fa-solid fa-arrow-left" style={{ fontSize: 11 }} /> actor
                </span>
              )}
            </div>
