...
export default async function ProgresoPage() {
  ...
  // Racha record (max of last 30 days)
  const rachaRecord = diasConsecutivos; // simplified — ideally would query max historical
  ...
  return (
    <div style={{ padding: "36px 40px 80px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{
        ...
        {/* XP + actividades */}
        <div style={{ borderRadius: 20, background: "#fff", border: "1px solid rgba(11,37,69,0.08)", padding: "24px", boxShadow: "0 2px 12px rgba(11,37,69,0.06)" }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#1E40AF", textTransform: "uppercase", letterSpacing: "0.12em", margin: "0 0 16px" }}>
            Estadísticas
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              ...
              <div key={item.label} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  <div style={{ fontSize: 18, fontWeight: 900, color: "#0B2545", letterSpacing: "-0.02em", lineHeight: 1 }}>{item.value}</div>
                  <div style={{ fontSize: 11, color: "rgba(11,37,69,0.45)", fontWeight: 500 }}>{item.label}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ borderRadius: 20, background: "#fff", border: "1px solid rgba(11,37,69,0.08)", padding: "24px", boxShadow: "0 2px 12px rgba(11,37,69,0.06)" }}>
          ...
          {stats.materiaMasFuerte ? (
            <div style={{ marginBottom: 16 }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: "rgba(11,37,69,0.40)", textTransform: "uppercase", letterSpacing: "0.10em", display: "block", marginBottom: 4 }}>
                Materia más fuerte
              </span>
              <span style={{ fontSize: 16, fontWeight: 900, color: "#0B2545" }}>{stats.materiaMasFuerte.nombre}</span>
            </div>
          ) : (
            <p style={{ fontSize: 14, color: "rgba(11,37,69,0.40)" }}>Completá actividades para ver estadísticas.</p>
          )}
          {stats.tipoActividades[0] && (
            <div>
              ...
              <span style={{ fontSize: 15, fontWeight: 800, color: "#0B2545" }}>
                {TIPO_CONFIG[stats.tipoActividades[0].tipo]?.label ?? stats.tipoActividades[0].tipo}
              </span>
              <span style={{ fontSize: 12, color: "rgba(11,37,69,0.40)", marginLeft: 6 }}>({stats.tipoActividades[0].cantidad} veces)</span>
            </div>
          )}
        </div>
      </div>

      {/* ── Bloque 2: Por materia ─── */}
      <div style={{ marginBottom: 32 }}>
        <h2 style={{ fontSize: 18, fontWeight: 900, color: "#0B2545", letterSpacing: "-0.03em", margin: "0 0 16px" }}>
          Por materia
        </h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 14 }}>
          {progresoUAC.map((uac) => {
            ...
            const r = 28;
            const c = 2 * Math.PI * r;
            const off = c * (1 - uac.pct / 100);
            return (
              <div key={uac.codigo} style={{
                borderRadius: 16,
                background: "#fff",
                ...
                <div style={{ position: "relative", flexShrink: 0 }}>
                  <svg width={64} height={64} viewBox="0 0 64 64" style={{ transform: "rotate(-90deg)" }}>
                    <circle cx={32} cy={32} r={r} fill="none" stroke="rgba(11,37,69,0.07)" strokeWidth={5} />
                    {uac.pct > 0 && (
                      <circle
                        cx={32} cy={32} r={r}
                  ...
                  <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <span style={{ fontSize: 11, fontWeight: 900, color: "#0B2545" }}>{uac.pct}%</span>
                  </div>
                </div>
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 800, color: "#0B2545", lineHeight: 1.25, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {uac.nombre.split(" ").slice(0, 3).join(" ")}
                  </div>
                  <div style={{ fontSize: 11, color: "rgba(11,37,69,0.40)", marginTop: 4 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 32 }} className="progreso-2col">
        ...
        <div style={{ borderRadius: 20, background: "#fff", border: "1px solid rgba(11,37,69,0.08)", padding: "24px", boxShadow: "0 2px 12px rgba(11,37,69,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <h2 style={{ fontSize: 15, fontWeight: 800, color: "#0B2545", margin: 0, letterSpacing: "-0.02em" }}>Racha diaria</h2>
            <div style={{ display: "flex", alignItems: "center", gap: 6, background: diasConsecutivos > 0 ? "rgba(251,146,60,0.10)" : "rgba(11,37,69,0.04)", border: diasConsecutivos > 0 ? "1px solid rgba(251,146,60,0.22)" : "1px solid rgba(11,37,69,0.08)", borderRadius: 999, padding: "4px 12px" }}>
              <span style={{ fontSize: 16 }}>🔥</span>
              <span style={{ fontSize: 16, fontWeight: 900, color: diasConsecutivos > 0 ? "#FB923C" : "rgba(11,37,69,0.25)", letterSpacing: "-0.02em" }}>{diasConsecutivos}</span>
            </div>
          </div>
          <p style={{ fontSize: 12, color: "rgba(11,37,69,0.45)", margin: "0 0 14px" }}>
            ...
            {calendario.map((day) => {
              ...
              return (
                <div key={day.fecha} title={day.fecha} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
                  <span style={{ fontSize: 8, color: "rgba(11,37,69,0.28)", fontWeight: 600 }}>
                    {DAY_LABELS[dow]}
                  </span>
                  <div style={{
                    width: "100%",
          ...
          <p style={{ fontSize: 11, color: "rgba(11,37,69,0.35)", margin: "12px 0 0" }}>
        ...
        <div style={{ borderRadius: 20, background: "#fff", border: "1px solid rgba(11,37,69,0.08)", padding: "24px", boxShadow: "0 2px 12px rgba(11,37,69,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <h2 style={{ fontSize: 15, fontWeight: 800, color: "#0B2545", margin: 0 }}>Logros</h2>
            <span style={{ fontSize: 12, color: "rgba(11,37,69,0.30)", cursor: "not-allowed" }}>Ver todos</span>
          </div>
          {logros.length === 0 ? (
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              <div style={{ fontSize: 44, marginBottom: 12 }}>🏆</div>
              <p style={{ fontSize: 14, fontWeight: 700, color: "#0B2545", margin: "0 0 6px" }}>Sin logros todavía</p>
      ...
      <div style={{ borderRadius: 20, background: "#fff", border: "1px solid rgba(11,37,69,0.08)", padding: "24px", boxShadow: "0 2px 12px rgba(11,37,69,0.06)" }}>
        <h2 style={{ fontSize: 15, fontWeight: 800, color: "#0B2545", margin: "0 0 18px" }}>Actividades recientes</h2>
        {actividadesRecientes.length === 0 ? (
          <p style={{ fontSize: 14, color: "rgba(11,37,69,0.40)", textAlign: "center", padding: "20px 0" }}>
              ...
              return (
                <div key={act.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#0B2545", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {act.titulo}
                    </div>
                    <div style={{ fontSize: 11, color: "rgba(11,37,69,0.40)", display: "flex", gap: 8 }}>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(11,37,69,0.35)" }}>{timeAgo(act.completadaEn)}</div>
                  </div>
                </div>
              );
            })}
