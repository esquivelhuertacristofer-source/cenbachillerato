 ...
 * de Semestre 1 servidas desde public/media/sem1/. Recorre cada subcarpeta
 ...
 * Idempotente: solo escribe si el valor cambia. No valida contra Zod —
 * mismo patrón que wire-infografias-actividades.ts (merge + update directo).
 *
 * Uso: npx tsx scripts/publish-media-sem1.ts
 */
...
const MEDIA_ROOT = resolve(process.cwd(), "public/media/sem1");
...
async function main() {
  ...
  for (const [carpeta, campo] of Object.entries(CARPETA_CAMPO)) {
    ...
    for (const archivo of archivos) {
      ...
      const rutaPublica = `/media/sem1/${carpeta}/${archivo}`;
  ...
  log(`\n✅ Publicación Sem1: ${ok} nuevas, ${sinCambio} ya al día, ${noEncontrada} sin actividad, ${fallo} fallidas.\n`);
