/**
 * Crea la cuenta demo de alumno para Semestre 4.
 * Uso: npx tsx scripts/create-demo-alumno-sem4.ts
 *
...
const DEMO_EMAIL    = "alumno-sem4@cenbachillerato-demo.com";
...
const DEMO_NOMBRE   = "Alumno Sem 4 Demo";
const DEMO_ESCUELA_CCT = "DEMO-001";
const DEMO_GRUPO_SEM4  = "Grupo 4A Demo";
...
async function getOrCreateGrupoSem4(sb: SupabaseClient<Database>, escuelaId: string): Promise<string> {
  const { data: existing } = await sb
    .from("grupos").select("id")
    .eq("nombre", DEMO_GRUPO_SEM4).eq("escuela_id", escuelaId).single();

  if (existing) {
    console.log(`  ↩  Grupo Sem 4 ya existe (id=${existing.id})`);
    return existing.id;
  }

  const { data, error } = await sb
    .from("grupos")
    .insert({ nombre: DEMO_GRUPO_SEM4, semestre: 4, escuela_id: escuelaId })
    .select("id").single();

  if (error || !data) throw new Error(`Error creando grupo Sem 4: ${error?.message}`);
  console.log(`  ✓  Grupo "${DEMO_GRUPO_SEM4}" creado (id=${data.id})`);
  return data.id;
}
...
async function upsertProfile(sb: SupabaseClient<Database>, userId: string, escuelaId: string): Promise<void> {
  ...
  console.log(`  ✓  Profile upsertado (semestre=4)`);
}
...
async function assignToGrupo(sb: SupabaseClient<Database>, userId: string, grupoId: string): Promise<void> {
  ...
  console.log(`  ✓  Alumno asignado a "${DEMO_GRUPO_SEM4}"`);
}
...
async function main() {
  ...
  console.log("\n🚀 CEN Bachillerato — Demo Alumno Semestre 4\n");

  console.log("1. Escuela demo:");
  const escuelaId = await getOrCreateEscuela(sb);

  console.log("\n2. Grupo Semestre 4:");
  const grupoId = await getOrCreateGrupoSem4(sb, escuelaId);
  ...
  console.log("\n" + "=".repeat(60));
  console.log("✅ LISTO — Cuenta demo Semestre 4 creada/verificada");
  console.log("=".repeat(60));
  console.log(`  Email:    ${DEMO_EMAIL}`);
  console.log(`  Password: ${DEMO_PASSWORD}`);
  console.log(`  Semestre: 4`);
  console.log(`  Grupo:    ${DEMO_GRUPO_SEM4}`);
  console.log("=".repeat(60) + "\n");
}
