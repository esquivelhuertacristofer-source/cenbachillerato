 ...
 *  1. «¿Necesidad o satisfactor?» — clasifica nueve tarjetas entre la carencia
 *     (necesidad) y el medio con que se cubre (satisfactor).
 *  2. «Empareja necesidad y satisfactor» — arrastra cada necesidad al medio
 *     (bien, servicio o relación) que la satisface, según los ejemplos del
 *     glosario A5.
 ...
 * clic-para-seleccionar / clic-para-colocar). Contenido VERBATIM de CS-II·P01.
 */
...
type Modo = "clasificar" | "emparejar" | "glosario" | "texto";
...
export function LabNecesidadesSatisfactores({ color }: PracticaLabProps) {
  ...
  const intentarTarj = (tarjId: string, bin: Categoria) => {
    ...
    if (t && t.categoria === bin) {
      ...
      if (Object.keys(ubicTarj).length + 1 >= TARJETAS.length) {
        sfxOk();
        persistMejor(true, emparejarDone, glosarioDone);
      }
    } else {
      setShakeTarj(bin);
      sfxNo();
  ...
  const intentarPar = (chipId: string, rowId: string) => {
    ...
    if (chipId === rowId) {
      ...
      setSelPar(null);
      sfxPlace();
      if (Object.keys(empPar).length + 1 >= PAREJAS.length) {
        sfxOk();
  ...
  const modosHechos = (clasificarDone ? 1 : 0) + (emparejarDone ? 1 : 0) + (glosarioDone ? 1 : 0) + (textoDone ? 1 : 0);
  ...
  const resetActual = modo === "texto" ? resetTexto : modo === "clasificar" ? resetClasificar : modo === "emparejar" ? resetEmparejar : resetGlosario;
  ...
  return (
    <div style={{ color: T.text }}>
      <style>{`
        ...
        .ns-tab { cursor:pointer; display:inline-flex; align-items:center; gap:9px; padding:10px 16px; border-radius:11px;
          border:1px solid ${T.line}; background:${T.glass}; color:${T.text2}; font-size:13.5px; font-weight:800; transition:all .14s; }
        .ns-tab:hover { border-color:${T.lineStrong}; color:#fff; }
        .ns-tab[data-on="true"] { border-color:${accent}; background:rgba(${color.rgba},0.16); color:#fff; box-shadow:0 0 16px -6px ${accent}; }
        .ns-icobtn { cursor:pointer; width:36px; height:36px; border-radius:9px; display:flex; align-items:center; justify-content:center;
          font-size:14px; border:1px solid ${T.line}; background:${T.glass}; color:rgba(255,255,255,0.7); transition:all .15s; }
        .ns-icobtn[data-on="true"] { background:rgba(${color.rgba},0.22); color:#fff; border-color:${accent}; }
        .ns-icobtn:hover { background:rgba(255,255,255,0.12); }
        .ns-chip { cursor:grab; display:inline-flex; align-items:center; justify-content:center; gap:8px; padding:11px 16px; border-radius:14px;
        ...
        .ns-chip:hover { border-color:${T.lineStrong}; background:rgba(255,255,255,0.09); }
        .ns-chip[data-sel="true"] { border-color:${accent}; background:rgba(${color.rgba},0.2); box-shadow:0 0 16px -5px ${accent}; }
        .ns-chip:active { cursor:grabbing; }
        .ns-row { border-radius:13px; border:1.5px solid ${T.line}; background:${T.glass}; padding:14px 16px; transition:all .16s; display:flex; align-items:center; gap:14px; flex-wrap:wrap; }
        .ns-row[data-shake="true"] { animation:nsShake .4s; border-color:${NO}; }
        .ns-row[data-done="true"] { border-color:${OK}66; background:${OK}0f; }
        .ns-slot { flex-shrink:0; min-width:200px; min-height:42px; border-radius:11px; border:1.5px dashed ${T.lineStrong}; background:${T.inset};
        ...
        .ns-slot[data-armed="true"] { border-color:${accent}; background:rgba(${color.rgba},0.1); }
        .ns-bin { border-radius:15px; border:1.5px solid ${T.line}; background:${T.glass}; padding:16px; transition:all .16s; min-height:260px; }
        ...
        .ns-btn:hover { border-color:${T.lineStrong}; }
        .ns-divider { height:1px; background:${T.line}; margin:18px 0; }
        @media (prefers-reduced-motion: reduce){ .ns-row[data-shake="true"], .ns-bin[data-shake="true"] { animation:none; } }

        /* Cajón de teoría */
        .ns-scrim { position:fixed; inset:0; background:rgba(2,8,20,0.55); backdrop-filter:blur(2px);
          opacity:0; pointer-events:none; transition:opacity .3s ease; z-index:60; }
        .ns-scrim[data-open="true"] { opacity:1; pointer-events:auto; }
        .ns-drawer { position:fixed; top:0; right:0; height:100dvh; width:min(560px,94vw); z-index:61;
          background:linear-gradient(180deg,#06182f 0%,#020d1d 100%); border-left:1px solid rgba(${color.rgba},0.32);
          box-shadow:-24px 0 60px -20px rgba(0,0,0,0.7); transform:translateX(102%); transition:transform .34s cubic-bezier(.4,0,.2,1);
          display:flex; flex-direction:column; }
        .ns-drawer[data-open="true"] { transform:translateX(0); }
        .ns-drawer-head { display:flex; align-items:center; justify-content:space-between; gap:12px;
          padding:18px 20px; border-bottom:1px solid ${T.line}; }
        .ns-drawer-body { overflow-y:auto; padding:20px; flex:1; }
        .ns-close { cursor:pointer; width:36px; height:36px; border-radius:10px; border:1px solid ${T.line};
          background:${T.glass}; color:#fff; font-size:15px; display:flex; align-items:center; justify-content:center; transition:all .15s; }
        .ns-close:hover { border-color:${accent}; background:rgba(${color.rgba},0.16); }
        .ns-teoria-fab { position:fixed; right:20px; bottom:20px; z-index:58; cursor:pointer; display:inline-flex; align-items:center; gap:9px;
          padding:11px 16px; border-radius:999px; border:1px solid ${accent}88; color:#fff; font-size:13px; font-weight:800;
          background:rgba(2,12,28,0.86); backdrop-filter:blur(10px); box-shadow:0 8px 28px -8px ${accent}; transition:all .16s; }
        .ns-teoria-fab:hover { background:rgba(${color.rgba},0.28); transform:translateY(-1px); }
        @media (max-width: 640px){ .ns-teoria-fab { right:12px; bottom:12px; padding:10px 13px; font-size:12px; } }
        ...
        .ns-chip { transition:transform .14s, box-shadow .14s, border-color .14s, background .14s; }
        .ns-chip:hover { transform:translateY(-2px); }
        .ns-chip[data-sel="true"] { transform:translateY(-3px) scale(1.02); }
        @media (prefers-reduced-motion: reduce){
          .ns-chip, .ns-chip:hover, .ns-chip[data-sel="true"] { transform:none; transition:none; }
        }
      `}</style>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 18 }}>
        {MODOS.map((m) => (
          <button key={m.id} className="ns-tab" data-on={modo === m.id} onClick={() => setModo(m.id)}>
        ...
        <button className="ns-icobtn" data-on={drawer} onClick={() => setDrawer(true)} title="Teoría de la práctica">
          <i className="fa-solid fa-book-open" />
        </button>
        <button className="ns-icobtn" data-on={sonido} onClick={toggleSonido} title={sonido ? "Silenciar" : "Activar sonido"}>
          <i className={`fa-solid ${sonido ? "fa-volume-high" : "fa-volume-xmark"}`} />
        </button>
        <button className="ns-icobtn" onClick={resetActual} title="Reiniciar este modo">
          <i className="fa-solid fa-rotate-left" />
        </button>
      </div>

      {/* ── Cajón de teoría ──────────────────────────────────────────── */}
      <button className="ns-teoria-fab" onClick={() => setDrawer(true)}>
        <i className="fa-solid fa-book-open" />
        Teoría
      </button>
      <div className="ns-scrim" data-open={drawer} onClick={() => setDrawer(false)} />
      <aside className="ns-drawer" data-open={drawer} aria-hidden={!drawer}>
        <div className="ns-drawer-head">
          <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
            <i className="fa-solid fa-book-open" style={{ color: accent, fontSize: 17 }} />
            <span style={{ fontSize: 15, fontWeight: 900, color: T.text }}>Teoría de la práctica</span>
          </div>
          <button className="ns-close" onClick={() => setDrawer(false)} title="Cerrar">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) clamp(300px,28vw,400px)", gap: 22, alignItems: "start" }}>
            ...
            <>
              <div style={{ ...card, padding: "18px 22px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12.5, fontWeight: 800, color: emparejarDone ? OK : T.text3 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                    {parLibres.map((p) => (
                      <button key={p.id} className="ns-chip" data-sel={selPar === p.id} onClick={() => setSelPar((v) => (v === p.id ? null : p.id))} {...dragProps(p.id)}>
                        <i className="fa-solid fa-hand-holding-heart" style={{ fontSize: 11, color: T.text3 }} />
                        {p.necesidad}
                      </button>
                    ))}
                  </div>
          ...
          {modo === "glosario" && (
            <EscribeTermino
              ...
              onCompletado={() => {
                ...
                sfxOk();
                persistMejor(clasificarDone, emparejarDone, true);
              }}
              onAcierto={sfxPlace}
              onError={sfxNo}
            />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              ...
              <div style={{ textAlign: "right", maxWidth: 180 }}>
                <div style={{ fontSize: 11.5, color: T.text3, lineHeight: 1.45 }}>
                  {bestEstrellas >= 3 ? "¡Distingues necesidades de satisfactores con claridad!" : "Termina los tres modos para ganar 2★; la tercera pide 2 errores o menos."}
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ borderRadius: 18, padding: "16px 18px", border: `1px solid rgba(${color.rgba},0.3)`, background: `rgba(${color.rgba},0.08)`, fontSize: 13, color: T.text2, lineHeight: 1.55, display: "flex", gap: 12 }}>
            ...
            <span>
              {modo === "clasificar" && (
                <>Una <strong style={{ color: T.text }}>necesidad</strong> es una carencia de algo indispensable; un <strong style={{ color: T.text }}>satisfactor</strong> es el bien, servicio o relación con que se cubre esa necesidad.</>
              )}
              {modo === "emparejar" && (
                <>Cada necesidad se cubre con un satisfactor: la <strong style={{ color: T.text }}>nutrición</strong> con el alimento, la <strong style={{ color: T.text }}>hidratación</strong> con el agua potable, la <strong style={{ color: T.text }}>salud</strong> con los servicios de salud.</>
              )}
              {modo === "glosario" && (
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {dentro.length === 0 ? (
                ...
                dentro.map((t) => (
                  <span key={t.id} style={{ animation: "nsPop .25s ease", display: "inline-flex", alignItems: "flex-start", gap: 7, padding: "8px 12px", borderRadius: 11, background: `${OK}1a`, border: `1px solid ${OK}55`, fontSize: 12.5, fontWeight: 700, color: "#fff", lineHeight: 1.4 }}>
        ...
        return (
          ...
          >
            <div className="ns-slot" data-armed={!done && !!selPar} style={done ? { borderStyle: "solid", borderColor: OK, background: `${OK}1a` } : undefined}>
              {done ? (
                <span style={{ animation: "nsPop .25s ease", fontSize: 13, fontWeight: 900, color: "#fff", display: "inline-flex", alignItems: "center", gap: 7 }}>
                  <i className="fa-solid fa-hand-holding-heart" />
                  {p.necesidad}
                </span>
              ) : (
                <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                  <i className="fa-solid fa-arrow-left" style={{ fontSize: 11 }} /> necesidad
                </span>
              )}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: done ? "#fff" : T.text2, lineHeight: 1.4 }}>{p.satisfactor}</div>
              <div style={{ fontSize: 11.5, color: T.text3, lineHeight: 1.4, marginTop: 3 }}>{p.ejemplo}</div>
            </div>
          </div>
        );
      })}
