 ...
 * de Semestre 3 servidas desde public/media/sem3/. Recorre cada subcarpeta
 ...
 * cambia. Mismo patrón que publish-media-sem2.ts (Sem3 tampoco tiene items
 * video_con_preguntas, por eso no aparece en el mapa).
 *
 * Uso: npx tsx scripts/publish-media-sem3.ts
 */
...
const MEDIA_ROOT = resolve(process.cwd(), "public/media/sem3");
...
const CARPETA_CAMPO: Record<string, string> = {
  ...
  // Sem3 no tiene actividades video_con_preguntas.
};
...
async function main() {
  ...
  for (const [carpeta, campo] of Object.entries(CARPETA_CAMPO)) {
    ...
    for (const archivo of archivos) {
      ...
      const rutaPublica = `/media/sem3/${carpeta}/${archivo}`;
  ...
  log(`\n✅ Publicación Sem3: ${ok} nuevas, ${sinCambio} ya al día, ${noEncontrada} sin actividad, ${fallo} fallidas.\n`);
